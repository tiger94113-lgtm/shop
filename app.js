...
const RPC_STORAGE_KEY = "assetMallPreferredRpcV1";
...
function getPreferredRpcUrl() {
  try {
    const value = localStorage.getItem(RPC_STORAGE_KEY);
    return value && CONFIG.rpcUrls.includes(value) ? value : "";
  } catch (error) {
    return "";
  }
}

function savePreferredRpcUrl(url) {
  if (!url) return;
  try {
...
function getRpcProbeOrder() {
  const preferred = getPreferredRpcUrl();
  if (!preferred) return [...CONFIG.rpcUrls];
  return [preferred, ...CONFIG.rpcUrls.filter((url) => url !== preferred)];
}

async function probeRpcProvider(url, timeoutMs = 2200) {
...
async function selectFastestRpc(urls, timeoutMs) {
  if (!urls.length) return null;

  const results = await Promise.allSettled(
    urls.map((url) => probeRpcProvider(url, timeoutMs))
  );
...
async function createProviderWithFallback() {
  const orderedUrls = getRpcProbeOrder();
  const preferredUrl = orderedUrls[0];

  if (preferredUrl) {
    try {
      const preferred = await probeRpcProvider(preferredUrl, 1400);
      savePreferredRpcUrl(preferred.url);
      console.log("复用上次成功 RPC:", preferred.url, "延迟", preferred.latencyMs + "ms");
      return preferred.provider;
    } catch (error) {
      console.warn("上次成功 RPC 当前不可用，开始重新测速:", preferredUrl, error.message);
    }
  }
  ...
  if (primary) {
    savePreferredRpcUrl(primary.url);
    console.log("已选用主力 RPC:", primary.url, "延迟", primary.latencyMs + "ms");
    return primary.provider;
  }

  const backup = await selectFastestRpc(backupPool, 3200);
  if (backup) {
    savePreferredRpcUrl(backup.url);
    console.log("已切换备用 RPC:", backup.url, "延迟", backup.latencyMs + "ms");
    return backup.provider;
  }

  throw new Error("RPC 节点池暂时不可用，请稍后重试或切换网络环境。");
}
...
function getCompactProductCode(value) {
      ...
      '<article class="product-card' + (product.id === state.selectedProductId ? ' active' : '') + '">' +
        ...
        '<div class="product-card-body">' +
          ...
          '<div class="product-card-stats">' +
            '<span>商品编号<strong>' + escapeHtml(getCompactProductCode(product.sku)) + '</strong></span>' +
      ...
      try {
        await provider.request({
          ...
          params: [{
            ...
            rpcUrls: CONFIG.rpcUrls.slice(0, 4),
